...
if __name__ == '__main__':
    ...
    for m in METRO:
        folder = '{}_edge{}_random/performance_under_different_M/metropolis_{}'.format(DATA, EDGE, m)
        ...
        for time in range(REPEAT):
            ...
            for r in REDUCE:
                ...
                nnOpt_random = copy.deepcopy(nnOpt)
                
                loss, acc, Hq_iter = nnOpt_random.FCSA(layer, reduce=r, metropolis=m, iteration = 0)
                
                loss_untrained.append(loss)
                acc_untrained.append(acc)

                losses_new, accs_new, epoch = nnOpt_random.fit(epoch_num=4,  # epoch
                                                            save_step=np.inf)
