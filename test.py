...
if __name__ == '__main__':
    ...
    parser.add_argument('--metro', type=str, default='20')
    parser.add_argument('--reduce', type=str, default='10,40,70,90') # 10,20,30,40,50,60,70,80,90,95,99,99.8
    ...
    for m in METRO:
        folder = '{}_edge{}_SQA/metropolis_{}'.format(DATA, EDGE, m)
        ...
        np.savetxt(os.path.join(folder, 'w1_0.txt'), nnOpt.model.fc1.weight.data.cpu().numpy())
        np.savetxt(os.path.join(folder, 'w2_0.txt'), nnOpt.model.fc2.weight.data.cpu().numpy())
        np.savetxt(os.path.join(folder, 'w3_0.txt'), nnOpt.model.fc3.weight.data.cpu().numpy())
        np.savetxt(os.path.join(folder, 'w4_0.txt'), nnOpt.model.fc4.weight.data.cpu().numpy())
        ...
        for time in range(REPEAT):
            ...
            for r in REDUCE:
                ...
                loss, acc, Hq_iter, data_iter = nnOpt_SQA.FCSQA(layer, reduce=r, metropolis=m, iteration=500)
                
                loss_untrained.append(loss)
                acc_untrained.append(acc)

                # losses_new, accs_new, epoch = nnOpt_SQA.fit(epoch_num=2,  # epoch
                #                                                     save_step=np.inf)
                # loss_trained.append(losses_new[-1])
                # acc_trained.append(accs_new[-1])
                
                np.savetxt(os.path.join(folder, 'w1_reduce{}.txt'.format(r*100)), nnOpt_SQA.model.fc1.weight.data.cpu().numpy())
                np.savetxt(os.path.join(folder, 'w2_reduce{}.txt'.format(r*100)), nnOpt_SQA.model.fc2.weight.data.cpu().numpy())
                np.savetxt(os.path.join(folder, 'w3_reduce{}.txt'.format(r*100)), nnOpt_SQA.model.fc3.weight.data.cpu().numpy())
                np.savetxt(os.path.join(folder, 'w4_reduce{}.txt'.format(r*100)), nnOpt_SQA.model.fc4.weight.data.cpu().numpy())
                
                np.savetxt(os.path.join(folder, 'm1_reduce{}.txt'.format(r*100)), nnOpt_SQA.model.fc1.mask.data.cpu().numpy())
                np.savetxt(os.path.join(folder, 'm2_reduce{}.txt'.format(r*100)), nnOpt_SQA.model.fc2.mask.data.cpu().numpy())
                np.savetxt(os.path.join(folder, 'm3_reduce{}.txt'.format(r*100)), nnOpt_SQA.model.fc3.mask.data.cpu().numpy())
                np.savetxt(os.path.join(folder, 'm4_reduce{}.txt'.format(r*100)), nnOpt_SQA.model.fc4.mask.data.cpu().numpy())
