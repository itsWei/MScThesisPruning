...
class NetworkOptimization(object):
    ...
    def link_reduce(self, model, layer, ratio):
        ...
        wanted_links = int((a1+a2) * ratio)

        mask_1_flat = mask_1.flatten()
        mask_2_flat = mask_2.flatten()
                
        mask_flat = torch.cat([mask_1_flat, mask_2_flat])
...
########################################################################
    ...
    def FCSA(self, layer, reduce, metropolis,iteration):
        ...
        msk_1, msk_2 = self.mask_ratio(layer)
        ...
        print('[+] round: {0} | metro: {1} | test acc: {2} | test loss: {3} | T: {4} | M1: {5}% | M2: {6}% '.format(
            i + 1, m + 1,
            np.round(accuracy, 3),
            np.round(losses.item(), 3),
            np.round(T, 3),
            np.round(msk_1 * 100, 3), 
            np.round(msk_2 * 100, 3)), end='\r')
...
########################################################################
    ...
    def FCSQA(self, layer, reduce, metropolis,iteration):
        ...
        losses = self.loss_func(outputs, test_label)
        accuracy = self.accuracy(outputs, test_label)
        print('[+] round: {0} | test acc: {1} | test loss: {2} | M1: {3}% | M2: {4}%'.format(
            i + 1,
            np.round(accuracy, 3),
            np.round(losses.item(), 3),
            np.round(msk_1 * 100, 3),
...
########################################################################
    ...
    def MINK(self, layer, reduce):
        ...
        print('[+] test acc: {0} | test loss: {1} | M1: {2}% | M2: {3}% '.format(
            np.round(accuracy, 3),
            np.round(losses.item(), 3),
            np.round(msk_1 * 100, 3), 
            np.round(msk_2 * 100, 3)), end='\r')
...
########################################################################
    ...
    def Test_Temp_minK(self, layer, mask_1_minK, mask_2_minK):
        
        test_data, test_label = next(iter(self.test))
        test_data = test_data.to(device=self.device)
        test_label = test_label.to(device=self.device)
        
        getattr(self.model, layer[0]).mask.data = torch.from_numpy(mask_1_minK)
        getattr(self.model, layer[1]).mask.data = torch.from_numpy(mask_2_minK)
        ...
        delta_loss = np.empty(1000)
        for i in range(1000):
            self.model = self.link_modify(self.model, layer, self.edge_changed)
        ...
        # delta = max(delta_loss) - min(delta_loss)
        delta = np.mean(delta_loss)
        
        print(delta)
        
        return delta
    
    def FCSQA_based_minK(self, layer, reduce, metropolis, iteration):
        test_data, test_label = next(iter(self.test))
        test_data = test_data.to(device=self.device)
        test_label = test_label.to(device=self.device)
        ...
        w = abs(np.concatenate([w_1.flatten(),w_2.flatten()]))
        msk = np.ones_like(w)
        
        N_initial = int((a1+a2)*(reduce+0.01))
        N_reduce = int((a1+a2)*reduce)
        
        sort_w = np.argsort(w)[0:N_initial]

        msk[sort_w[0:N_reduce]] = 0
        mask_1_minK = msk[0:a1].reshape(w_1.shape)
        mask_2_minK = msk[a1:a1+a2].reshape(w_2.shape)
        
        T = self.Test_Temp_minK(layer, mask_1_minK, mask_2_minK)
        # T = 0.002
        n = int(min(reduce+0.005,1-reduce-0.005)*(a1+a2))
        Tau = metropolis*T*np.arctanh(1/(4*n)**(1/(2*n)))
        J_plus = T/2 * np.log(coth(Tau / (metropolis*T)))
        
        sigma = np.zeros([metropolis+1,a1+a2])
        Hq = H_loss = np.empty(metropolis)
        
        for m in range(metropolis):
            np.random.shuffle(sort_w)
            msk = np.ones_like(w)
            msk[sort_w[0:N_reduce]] = 0
            sigma[m,:] = np.intc(msk.flatten()*2 - 1)
            
            mask_1_init = msk[0:a1].reshape(w_1.shape)
            mask_2_init = msk[a1:a1+a2].reshape(w_2.shape)
        ...
        for i in np.arange(iteration):
            ...
            for m in range(metropolis):
                mask = np.intc((sigma[m,:]+1)/2)
                mask_1_iter = mask[0:a1].reshape(w_1.shape)
                mask_2_iter = mask[a1:a1+a2].reshape(w_2.shape)
                ...
                mask_new = np.concatenate([mask_1_new.flatten(),mask_2_new.flatten()])
        ...
        Min_state = sigma[np.where(H_loss == Min_energy)[0][0],:]
        mask_best = np.intc((Min_state+1)/2)   
        mask_1_best = mask_best[0:a1].reshape(w_1.shape)
        mask_2_best = mask_best[a1:a1+a2].reshape(w_2.shape)
